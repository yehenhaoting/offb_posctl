...
int main(int argc, char **argv)
{
    ...
    //vicon 数据
    ros::Subscriber position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/uam_base/pose", 10, pos_cb);
    ros::Subscriber velocity_sub = nh.subscribe<geometry_msgs::TwistStamped>("/mocap/vel", 10, vel_cb);

 // 【发布】飞机姿态/拉力信息 坐标系:NED系
    ros::Publisher attitude_pub = nh.advertise<mavros_msgs::AttitudeTarget>("/cmd/setpoint_attitude", 10);
    ...
    PIDX.set_sat(0.1, 3, 0.0);
    PIDY.set_sat(0.1, 3, 0.0);
    PIDZ.set_sat(1.5, 5, 0.0);

    // 设置速度环积分上限 控制量最大值 误差死区
    PIDVX.set_sat(0.1, 1.5, 0);
    PIDVY.set_sat(0.1, 1.5, 0);
...
//  计算最终的输出值
    angle_target.x = satfunc((angle_des.x + 0.0*param.DOB_rate * angle_dis[0]), 0.314f, 0.0f);
    angle_target.y = satfunc((angle_des.y + 0.0*param.DOB_rate * angle_dis[1]), 0.314f, 0.0f);
...
//    attitude_target.orientation = euler2quaternion(angle_target.x, angle_target.y, angle_target.z);
    attitude_target.orientation.x = angle_target.x;
    attitude_target.orientation.y = angle_target.y;
//    attitude_target.orientation.x = 0;
//    attitude_target.orientation.y = 0;
    attitude_target.thrust = thrust_target;
    ROS_INFO_STREAM(attitude_target);
    return 0;
}
